...
class SkyViewWidget(QtWidgets.QWidget):
    ...
    def update_plot(self, azimuth, elevation, passes=None):
        ...
        ax.set_xticklabels(
            ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'], color='white')
            ...
            for pass_data in passes:
                if len(pass_data['azimuths']) > 3:  # Минимум 4 точки для интерполяции
                    ...
                    if len(split_indices) > 0:
                        ...
                        segments = np.split(
                            np.column_stack((az, el)), split_indices)
                    else:
                    ...
                    for segment in segments:
                        if len(segment) > 3:
                            ...
                            try:
                                ...
                                az_new = np.linspace(
                                    az_seg.min(), az_seg.max(), 100)
                                el_new = spline(az_new)

                                ax.plot(az_new, el_new, 'lime',
                                        linewidth=2, alpha=0.7)
                            except:
                                ax.plot(az_seg, el_seg, 'lime',
                                        linewidth=2, alpha=0.7)
